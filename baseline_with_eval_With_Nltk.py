...
def preprocessDataNelec(dataFilePath, mode, preprocess=False):
    """Load data from a file, process and return indices, conversations and labels in separate lists
    Input:
        dataFilePath : Path to train/test file to be processed
        mode : "train" mode returns labels. "test" mode doesn't return labels.
...
def preprocessDataWithoutText(dataFilePath, mode):
    """Load data from a file, process and return indices, conversations and labels in separate lists
    Input:
        dataFilePath : Path to train/test file to be processed
        mode : "train" mode returns labels. "test" mode doesn't return labels.
    ...
    with io.open(dataFilePath, encoding="utf8") as finput:
        ...
        for line in finput:
            ...
            allchars = [str for str in line]
            repeatedChars = ['.', '?', '!', ',']
            # repeatedChars=repeatedChars+list(string.ascii_lowercase)+list(string.ascii_uppercase)
            for c in repeatedChars:
                lineSplit = line.split(c)
                while True:
                    try:
...
def getEmbeddingMatrixEmoji(wordIndex):
    """Populate an embedding matrix using a word-index. If the word "happy" has an index 19,
       the 19th row in the embedding matrix should contain the embedding vector for the word "happy".
    Input:
        wordIndex : A dictionary of (word : index) pairs, extracted using a tokeniser
    ...
    embeddingsIndex = {}
    # Load the embedding vectors from ther GloVe file
    with io.open(os.path.join(emojiDir, 'emoji_embeddings_300d.txt'), encoding="utf8") as f:
        for line in f:
            values = line.split()
            word = values[0]
            embeddingVector = np.asarray(values[1:], dtype='float32')
